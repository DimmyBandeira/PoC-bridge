...
class WhatsAppProvider(BaseProvider):
    async def dispatch(self, event: dict[str, Any]) -> dict[str, Any]:
        logger.info("[MOCK] WhatsApp dispatch acionado para parceiro=%s", event.get("partner"))
        return {
            "provider": self.name,
    ...
    async def cancel(self, payload: dict[str, Any]) -> dict[str, Any]:
        logger.info("[MOCK] WhatsApp cancel acionado para id=%s", payload.get("dispatch_id"))
